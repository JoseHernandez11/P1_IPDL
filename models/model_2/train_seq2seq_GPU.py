...
# Definición clase decoder con atención de Bahdanau y  una única capa oculta
...
# Función de entrenamiento (modificada para ir guardando el mejor modelo conforme va entrenando)
def train(train_dataloader, encoder, decoder, n_epochs, learning_rate=0.001,
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        save_flag = False
        # Check if the current epoch's loss has improved
        ...
        # Save the model if it's the first epoch, every 5 epochs, or if the loss improved
        if save_directory and (epoch == 1 or epoch % 20 == 0 or save_flag):
            os.makedirs(save_directory, exist_ok=True)
            
            # Save as 'encoder_best_model.pt' only if the loss improved or it's the first epoch
            ...
            # Save with epoch in filename for every 20 epochs
            elif epoch % 20 == 0:
    ...
    if save_directory:
        ...
        train_history_df["epoch"]= list(range(1, n_epochs + 1))
        train_history_df["loss"] = epoch_losses
        csv_path = os.path.join(save_directory, "train_history.csv")
        train_history_df.to_csv(csv_path, index=False)
...
train(train_dataloader, encoder, decoder, epochs, print_every=1)
end_time = time.time()
print(f"Training time: {end_time-start_time} s")

### Imprimimos unos ejemplos al terminar de entrenar para ver que todo ha ido bien

print(evaluateRandomly(encoder, decoder))